  ...
  <div class="body">
    ...
    <div class="bd">
      <van-list v-model:loading="loading" :finished="finished" :disabled="true">
        <van-cell v-for="item, index in songs" :key="item.name">
...
const props = defineProps(['songs', 'playlistDetail'])
const loading = ref(false)
const finished = ref(true)


const bofang = (i) => {
  if (props.playlistDetail) { // 加入判断，防止出现空指针错误
    playBar.songs = props.songs
    playBar.playlist = props.playlistDetail
    playBar.index = i
    playBar.isPlay = true
  }
}
